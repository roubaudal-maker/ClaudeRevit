...
public class CopyElements : IRevitTool
{
    public string Name => "copy_elements";

    public string Description =>
        "Copies one or more elements with a translation vector (in feet). Returns the new element ids.";
    ...
    public InputSchema InputSchema => new()
    {
        Properties = new Dictionary<string, JsonElement>
        {
            ...
            ["dx_ft"] = JsonSerializer.SerializeToElement(new { type = "number", description = "Copy translation X in feet." }),
            ["dy_ft"] = JsonSerializer.SerializeToElement(new { type = "number", description = "Copy translation Y in feet." }),
            ["dz_ft"] = JsonSerializer.SerializeToElement(new { type = "number", description = "Copy translation Z in feet." })
        },
    ...
    public string Execute(IReadOnlyDictionary<string, JsonElement> input, UIApplication app)
    {
        ...
        if (Math.Abs(dx) < 1e-9 && Math.Abs(dy) < 1e-9 && Math.Abs(dz) < 1e-9)
            throw new InvalidOperationException("Translation vector is zero — copies would overlap originals.");
        ...
        return JsonSerializer.Serialize(new
        {
            ...
            new_ids = newIds.Select(i => i.Value).ToList(),
